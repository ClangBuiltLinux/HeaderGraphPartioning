...
def compute_proximity(
    header_filename: Path, c_filenames: Path
...
def create_distance_matrix(
    proximity: Dict[Tuple[str, str], int], symbols: List[str]
) -> np.ndarray:
...
def hierarchical_clustering(
    proximity: Dict[Tuple[str, str], int], header_filename: Path, method_name: str
):
    ...
    symbol_to_cluster = {
        symbols[i]: cluster_id for i, cluster_id in enumerate(clusters)
    }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="""This script finds the occurrences of
    ...
    parser.add_argument(
        "-f", "--header_filename", type=Path, required=True, help="Path to header"
    )
    parser.add_argument(
        "-m",
        "--method",
    ...
    parser.add_argument("-d", action="store_true", help="Debug mode on.")
    ...
    if DEBUG:
        sorted_data = sorted(
            [(count, sym1, sym2) for (sym1, sym2), count in proximity.items()],
            reverse=True,
        )
